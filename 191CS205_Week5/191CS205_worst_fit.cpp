...
int insert_p(vector<pair<int,int>> &partition, int p_no, int p_size, int size, int remaining){
  ...
  if(partition.size() == 1 && partition[0].first == -1){
    ...
    for(i=0; i<partition.size(); i++){
      ...
      if(partition[i].second >= p_size){
        if(worst==-1) worst = i;
          ...
          if(partition[i].second > partition[worst].second){
            worst = i;
          }
        }
      }
    ...
    remaining -= p_size;
    rem_part = partition[worst].second - p_size;    
    partition[worst].second = p_size;
    partition[worst].first = p_no;

    if(rem_part > 0){
      if(worst < partition.size()-1)
        partition.insert(partition.begin()+worst+1, make_pair(-1, rem_part));
      else
