...
int insert_p(vector<pair<int,int>> &partition, int p_no, int p_size, int size, int remaining){
  ...
  if(partition.size() == 1 && partition[0].first == -1){
    ...
    for(i=0; i<partition.size(); i++){
      ...
      if(partition[i].second >= p_size){
        if(best==-1) best = i;
          ...
          if(partition[i].second < partition[best].second){
            best = i;
          }
        }
      }
    ...
    remaining -= p_size;
    rem_part = partition[best].second - p_size;    
    partition[best].second = p_size;
    partition[best].first = p_no;

    if(rem_part > 0){
      if(best < partition.size()-1)
        partition.insert(partition.begin()+best+1, make_pair(-1, rem_part));
      else
